...
<html>
    <head>
    <title>
        Sign up</title><link rel="icon" href="book.jpg" type="image/x-icon">
</head>
...
$row = mysqli_fetch_array($result);
    echo $row["id"];
    echo $row["mbname"];
    $d = $row['date_of_birth'];  // echo $row["date_of_birth"] = date_format("m/d/Y");
...
?>
 ...
 <div class="bg" style="width:30%;" >
        ...
        <font>
                   ...
                   <input type="text" value="<?php  echo $row["mbname"]; ?>" name="name" autocomplete="on"  placeholder="Enter Name" style=" text-transform :uppercase"  required><br>
 
    


                   <label>Date of Birth<sup>*</sup></label>
                   <input  type="date"  value="<?php  $var = $row['date_of_birth'];echo date("m/d/Y", strtotime($var) );  ?>" name="dateofbirth"  autocomplete="on"  placeholder="Enter Date of Birth" required><br>
...
echo $Password;
            ...
            //check file extension
                if ($formOk && !in_array($type, array('image/png', 'image/x-png', 'image/jpeg', 'image/pjpeg', 'image/gif'))) {
                 ...
                 if ($formOk && filesize($path) > 500000) {
                    $formOk = false;
                    echo "0Error: File size must be less than 500 KB.";
                }
        ...
        $content = mysqli_real_escape_string($conn, $content);
          ...
          $sql="INSERT INTO `sineup`(`id`, `mbname`, `date_of_birth`, `gender`, `phone no.`, `email`, `address`, `city`, `state`, `country`, `upload_img`, `username`, `password`,`status`) VALUES ('','$name','$dateofbirth','$gender','$mobilenumber','$email','$Address','$City','$State','$Country','$content','$Username','$Password','no')";

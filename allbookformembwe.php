...
<div class ="main">
    ...
    <center>
       ...
       <input  placeholder="Search Book by name" type="text"  name="txtsearch"><br>
...
<?php 
         ...
         echo   $book = $_POST['category'];
        
           echo $txtsearch = mysqli_real_escape_string($conn,$_POST['txtsearch']);
           // echo  $txtsearch = $_POST['txtsearch']; 
      
           //$sql = "SELECT * FROM `libsineup` WHERE `id` = $txtsearch";
    echo "</center><br><br>";
           $sql ="SELECT * FROM `add_book` WHERE `$book` LIKE  '%$txtsearch%'  AND `status` LIKE 'yes' "; 
...
echo "<tr class='tbl-content'>
       ...
       <td>"; echo $row["publisher"];echo"<br></td>
          ...
          {
            $sql = "SELECT * FROM `add_book`  WHERE `status` LIKE 'yes'";$result = mysqli_query($conn, $sql);
//$row = mysqli_fetch_array($result);

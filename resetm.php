...
{  $id=$_GET['id'];
 ...
 // $select=mysql_query("select email,password from user where md5(email)='$email' and md5(password)='$pass'");
  $sql="SELECT * FROM `sineup` WHERE  md5(`email`) = '$email' AND md5(`password`) = '$pass'AND md5(`id`) = '$id' ";
...
<div id="fullpage" >
    ...
    <center>
        <font>       
        <form name="form" action="changepassforgotm.php" method="post" enctype="multipart/form-data"> <div class="bg">Change password</div>
